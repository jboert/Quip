...
final class CloudflareTunnel {
    ...
    private static let logPath = "/tmp/voicecode_tunnel.log"

    func start(localPort: UInt16 = 8765) {
        guard !isRunning else { return }

        let cfPath = "/opt/homebrew/bin/cloudflared"
        guard FileManager.default.fileExists(atPath: cfPath) else { return }

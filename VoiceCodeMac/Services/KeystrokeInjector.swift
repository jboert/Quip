...
// VoiceCodeMac — Sends text and keystrokes to terminal windows via AppleScript
...
final class KeystrokeInjector {
    ...
    ///   - windowId: VoiceCode window identifier (used for logging; windowIndex targets the window)
    ...
    @discardableResult
    func sendText(_ text: String, to windowId: String, pressReturn: Bool, terminalApp: TerminalApp, windowIndex: Int = 1) -> InjectionResult {
        ...
        case .terminal:
            if pressReturn {
                // `do script` in Terminal.app executes and presses Return
                script = """
                tell application "Terminal"
                    do script "\(escapedText)" in window \(windowIndex)
                end tell
                """
            } else {
                // Use System Events keystroke for text without Return
                script = """
    ...
    ///   - windowId: VoiceCode window identifier
    ///   - terminalApp: Which terminal emulator to target
